---
title: Configuring Ganpa
---

import Tabs from '@theme/Tabs'; import TabItem from '@theme/TabItem';

> This is the guide on how to configure **Ganpa v5 self-hosted**.

## Must configure

### Database

In order for Ganpa server to work, you need a running database and its connection details. See
the [database configuration section](#database-configuration) for the available options and configuration details.

## Nice to configure

### Ganpa URL {#unleash-url}
...
In order to customize "anything" in Ganpa you need to
use [Ganpa from Node.js](./getting-started.md#option-two---from-nodejs) or start
...
**Available Ganpa options include:**
...
- **authentication** - (object) - An object for configuring/implementing custom admin authentication
    ...
    - `type` / `AUTH_TYPE`: `string` — What kind of authentication to use. Possible values
        ...
        - `none` - _Deprecated_ Turn off authentication completely. If no API token is provided towards /`api/client`
          ...
          use the `demo` type for simple, insecure usage of Ganpa. This auth type has many known limitations,
          particularly related to personalized capabilities such as favorites
          and [notifications](../../reference/notifications.md).
    - `customAuthHandler`: function `(app: any, config: IGanpaConfig): void` — custom express middleware handling
    ...
    - `initApiTokens` / `INIT_ADMIN_API_TOKENS`, `INIT_CLIENT_API_TOKENS`,
      ...
      client, and frontend tokens respectively, and Ganpa will assign usernames automatically.

    -  `demoAllowAdminLogin` / `AUTH_DEMO_ALLOW_ADMIN_LOGIN`: `boolean` — Allows you to log in as the admin user when running Ganpa with the `demo` auth type. To log in as the admin user, use `admin` as your email. Defaults to `false`.
...
- **disableLegacyFeaturesApi** (boolean) - whether to disable
  ...
  to `false` (`DISABLE_LEGACY_FEATURES_API`). Introduced in Ganpa 4.6.
- **email** - the email object configuring an SMTP server for sending welcome mails and password reset mails
    ...
    - `sender` - Which email should be set as sender of mails being sent from Ganpa?
    - `smtpuser` - Username for your SMTP server
    - `smtppass` - Password for your SMTP server
- ~~eventHook~~ (`function(event, data)`) - (_deprecated in Ganpa 4.3_ in favor of
  the [Webhook integration](/reference/integrations/webhook.md). **Removed in Ganpa 5**) If provided, this function
...
- **preRouterHook** (function) - use this to register custom express middlewares before the `unleash` specific routers
  are added.
- **secureHeaders** (boolean) - use this to enable security headers (HSTS, CSP, etc) when serving Ganpa from HTTPS.
...
- **server** - The server config object taking the following properties
    ...
    - _unleashUrl_ (string) - Used to specify the official URL this instance of Ganpa can be accessed at for an end
      user. Can also be configured through the environment variable `UNLEASH_URL`.
    - _gracefulShutdownEnable_: (boolean) - Used to control if Ganpa should shutdown gracefully (close connections,
      stop tasks,). Defaults to true. `GRACEFUL_SHUTDOWN_ENABLE`
    - _gracefulShutdownTimeout_: (number) - Used to control the timeout, in milliseconds, for shutdown Ganpa
    ...
    - _enableScheduledCreatedByMigration_: (boolean) - Schedules migrations that fills the field created_by_user_id for
      ...
      application's performance. Defaults to false. **NOTE:** If your installation started with Ganpa v5.8 or later,
...
- **session** - The session config object takes the following options:
    ...
    - _clearSiteDataOnLogout_ (boolean) - When `true`, a logout action will return a Clear Site Data response header
      instructing the browser to clear all cookies on the same domain Ganpa is running on. If disabled unleash will
...
- **frontendApi** - Configuration options for the [Ganpa front-end API](/reference/front-end-api.md).
...
- **responseTimeWithAppNameKillSwitch** - use this to disable metrics with app names. This is enabled by default but may
  increase the cardinality of metrics causing Ganpa memory usage to grow if your app name is randomly generated (which
...
If you're using Ganpa as part of a larger express app, you can disable the automatic server start by
...
const start = async () => {
    ...
    console.log(`Ganpa app generated and attached to parent application`);
};
...
> PS! Ganpa will listen for the `SIGINT` signal and automatically trigger graceful shutdown of Ganpa.

If you need to stop Ganpa (close database connections, and stop running Ganpa tasks) you may use the stop function.
Be aware that it is not possible to restart the Ganpa instance after stopping it, but you can create a new instance of
Ganpa.
...
const start = async () => {
    ...
    //Sometime later
    await instance.stop();
    console.log('Ganpa has now stopped');
};
...
Changing segment limits could have a negative impact on the performance of Ganpa SDKs and cause network congestion.
...
you change the [segment limits](/reference/segments.mdx#segment-limits) that Ganpa uses.
...
## Securing Ganpa {#securing-unleash}

You can integrate Ganpa with your authentication provider (OAuth 2.0). Read more
...
possible to swap out the logger provider (only when using Ganpa programmatically). You do this by providing an
...
environment variables and in code with the Ganpa options object, Ganpa will use the in-code username.

:::

You cannot run the Ganpa server without a database. You must provide Ganpa with database connection details for it
to start correctly.

The available options are listed in the table below. Options can be specified either via JavaScript (only when starting
Ganpa via code) or via environment variables. The "property name" column below gives the name of the property on the
Ganpa options' `db` object.
...
| Property name     | Environment variable | Default value | Description                                                                                                                                                                                                                                           |
|-------------------|----------------------|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `databaseUrl`     | `DATABASE_URL`       | N/A           | A string that matches a single-host [libpq connection string](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING), such as `postgres://USER:PASSWORD@HOST:PORT/DATABASE`. Ganpa does **not** support using multiple hosts. |
...
When initializing Ganpa from code, you'll provide the `db.ssl` option as a JavaScript object. As such, any functions
...
If you want to read content from a file, you should either initialize Ganpa via JavaScript or manually interpolate the
...
Ganpa builds directly on the [node-postgres library](https://node-postgres.com/features/ssl), so we support all the
...
reason, Ganpa cannot support all of Postgres' SSL modes. Specifically, Ganpa **does not support** `sslmode=prefer`.
...
Ganpa SAAS runs on db.t4g.small and we're happy with that.
...
If you're on Ganpa 5.9.0 or newer, you might encounter issues with metrics from connected Edge instances. There are
...
- If you absolutely cannot upgrade your Edge instances to v17.0.0 or newer you can start Ganpa with the environmental
...
### Proxying requests from Ganpa

You can configure proxy services that intercept all outgoing requests from Ganpa. This lets you use the Microsoft
