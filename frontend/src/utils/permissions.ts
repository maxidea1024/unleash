...
import { ENVIRONMENT_PERMISSION_TYPE, PROJECT_PERMISSION_TYPE } from '@server/util/constants';
import type {
  IPermission,
  ICheckedPermissions,
  IPermissionCategory,
...
export const getRoleKey = (permission: IPermission): string => {
  return permission.environment ? `${permission.name}-${permission.environment}` : `${permission.name}`;
};

export const permissionsToCheckedPermissions = (permissions: IPermission[]): ICheckedPermissions =>
  permissions.reduce((checkedPermissions: { [key: string]: IPermission }, permission: IPermission) => {
...
export const getCategorizedRootPermissions = (permissions: IPermission[]) => {
  ...
  const categories = rootPermissions.reduce((categories: IPermissionCategory[], permission) => {
    const categoryLabel =
      ROOT_PERMISSION_CATEGORIES.find((category) => category.permissions.includes(permission.name))?.label || 'Other';
  ...
  return categories
    .map((category) => ({
      ...category,
      permissions: [...new Set(category.permissions.sort((a, b) => a.displayName.localeCompare(b.displayName)))],
    }))
    .sort(sortCategories);
};

export const getCategorizedProjectPermissions = (permissions: IPermission[]) => {
  const projectPermissions = permissions.filter(({ type }) => type === PROJECT_PERMISSION_TYPE);
  const environmentPermissions = permissions.filter(({ type }) => type === ENVIRONMENT_PERMISSION_TYPE);
  ...
  categories.push(
    ...environmentPermissions.reduce((categories: IPermissionCategory[], permission) => {
...
) => [...projectPermissions, ...environmentPermissions.flatMap(({ permissions }) => permissions)];

const sortCategories = ({ label: aLabel }: IPermissionCategory, { label: bLabel }: IPermissionCategory) => {
