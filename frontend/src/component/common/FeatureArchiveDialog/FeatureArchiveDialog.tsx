...
const UsageWarning = ({
  ...
  if (ids) {
    return (
      <Alert severity={'warning'} sx={{ m: (theme) => theme.spacing(2, 0) }}>
        <Typography fontWeight={'bold'} variant={'body2'} display='inline'>
        ...
        <ul>
          {ids?.map((id) => (
            <li key={id}>{<Link to={formatPath(id)}>{id}</Link>}</li>
          ))}
        </ul>
      </Alert>
    );
      ...
      <Alert severity='warning' sx={{ m: (theme) => theme.spacing(2, 0) }}>
        ...
        <ul>
          {changeRequests.map(({ id, title }) => {
            const text = title ? `#${id} (${title})` : `Change request #${id}`;
            return (
              <li key={id}>
                <Link
  ...
  useEffect(() => {
    if (isOpen) {
      verifyArchiveFeatures(projectId, featureIds)
        .then((res) => res.json())
        .then(({ hasDeletedDependencies, parentsWithChildFeatures }) => {
