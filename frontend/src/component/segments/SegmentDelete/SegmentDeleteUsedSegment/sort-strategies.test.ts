...
describe('sorting strategies by feature', () => {
  test('strategies with the same id are sorted: existing first, then change requests', () => {
    ...
    const changeRequestStrategies = [{ id: 'a', featureName: 'feature1', changeRequest: { id: 1 } }];
    ...
    expect(sortStrategiesByFeature(strategies, changeRequestStrategies)).toStrictEqual(expected);
  });
  ...
  test('the same strategy used in multiple change requests is sorted by change request id', () => {
    ...
    expect(sortStrategiesByFeature([], changeRequestStrategies)).toStrictEqual(expected);
  });
