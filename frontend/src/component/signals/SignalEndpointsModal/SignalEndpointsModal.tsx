...
export const SignalEndpointsModal = ({
  ...
  const formatApiCode =
    () => `curl --location --request ${editing ? 'PUT' : 'POST'} '${uiConfig.unleashUrl}/api/admin/signal-endpoints${editing ? `/${signalEndpoint.id}` : ''}' \\
    ...
    try {
      if (editing) {
        ...
        if (tokenGeneration === TokenGeneration.NOW) {
          const { token } = await addSignalEndpointToken(signalEndpoint.id, {
            name: tokenName,
          });
