...
describe('useFilteredFlagTrends', () => {
  it('should return all project flag trends when all projects option is selected', () => {
    const projects = ['*'];
    const { result } = renderHook(() => useFilteredTrends(mockProjectFlagTrends, projects));
  ...
  it('should re-render if input has changed', () => {
    const projects = ['project1'];
    const { result, rerender } = renderHook(({ input, projects }) => useFilteredTrends(input, projects), {
      initialProps: {
