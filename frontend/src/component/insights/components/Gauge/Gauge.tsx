...
const describeArc = (radius: number, startAngle: number, endAngle: number) => {
  ...
  const dSvgAttribute = ['M', start.x, start.y, 'A', radius, radius, 0, largeArcFlag, 0, end.x, end.y].join(' ');
...
const GaugeText = () => {
  ...
  return (
    <>
      ...
      <text textAnchor='middle' transform='translate(0 -1.25)' fontSize={fontSize} fill='currentColor'>
        Medium
      </text>
      <text transform='translate(1.15 -0.5) rotate(65)' textAnchor='middle' fontSize={fontSize} fill='currentColor'>
        Fast
      </text>
    </>
  );
...
export const Gauge: VFC<GaugeProps> = ({ value, min = 0, max = 100 }) => {
  ...
  const valueAngle = startAngle + (endAngle - startAngle) * normalizeValue(value || 0, min, max);
  ...
  return (
    ...
    >
      <svg width='250' height='150' viewBox={[-1.5, -1.5, 3, 1.5].join(' ')} style={{ overflow: 'visible' }}>
        ...
        <defs>
          ...
          >
            <stop offset='0%' stopColor={theme.palette.charts.gauge.gradientStart} />
            <stop offset='100%' stopColor={theme.palette.charts.gauge.gradientEnd} />
          </linearGradient>
        </defs>
        <path
          d={backgroundArcPath}
