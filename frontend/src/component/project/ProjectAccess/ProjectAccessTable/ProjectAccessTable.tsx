...
export const ProjectAccessTable: FC = () => {
        ...
        Cell: ({
          ...
          <ActionCell>
            <PermissionIconButton
              ...
              projectId={projectId}
              to={`edit/${row.type === ENTITY_TYPE.GROUP ? 'group' : 'user'}/${row.entity.id}`}
  ...
  const removeAccess = async (userOrGroup?: IProjectAccess) => {
    ...
    try {
      ...
      setToastData({
        type: 'success',
        title: `${name || `The ${entityType}`} has been removed from project`,
      });
    } catch (err: any) {
      setToastData({
        type: 'error',
        title: err.message || `Server problems when removing ${entityType}.`,
      });
    }
    setRemoveOpen(false);
  };
