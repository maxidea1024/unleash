...
export const ProjectDefaultStrategyForm = ({
  ...
  const { uiConfig, error: uiConfigError, loading: uiConfigLoading } = useUiConfig();
  ...
  const validateParameter = (name: string, value: IFeatureStrategyParameters[string]): boolean => {
    ...
    if (name !== 'groupId') {
      const parameterValueError = validateParameterValue(parameter, value);
  ...
  return (
    <StyledForm onSubmit={onSubmitWithValidation}>
      ...
      />
      <ConditionallyRender
        condition={strategy.parameters != null && 'stickiness' in strategy.parameters}
        show={
          <StrategyVariants
            strategy={strategy}
            setStrategy={setStrategy}
      ...
      <StyledButtons>
        <PermissionButton
          ...
          color='primary'
          type='submit'
          disabled={loading || !hasValidConstraints || errors.hasFormErrors()}
