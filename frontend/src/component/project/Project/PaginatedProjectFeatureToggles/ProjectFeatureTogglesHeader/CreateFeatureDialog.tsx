...
const CreateFeatureDialogContent = ({
  ...
  const formatApiCode = () => {
    return `curl --location --request POST '${uiConfig.unleashUrl}/api/admin/projects/${project}/features' \\
          ...
          configButtons={
            <>
              ...
              <MultiSelectConfigButton<ITag>
                ...
                onChange={setTags}
                button={{
                  label: tags.size > 0 ? `${tags.size} selected` : 'Tags',
                ...
                onOpen={() => setDocumentation(configButtonData.tags)}
                onClose={clearDocumentationOverride}
              />
              <SingleSelectConfigButton
                tooltip={{
                ...
                options={featureTypes.map((type: FeatureTypeSchema) => ({
                  label: type.name,
                  value: type.id,
                }))}
                onChange={(value: any) => {
              ...
              <ToggleConfigButton
                tooltip={{
                  ...
                  description: configButtonData.impressionData.text,
                }}
                currentValue={impressionData}
                onClick={() => setImpressionData(!impressionData)}
