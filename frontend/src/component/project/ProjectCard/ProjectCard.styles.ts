...
export const StyledProjectCard = styled(Card)<{ disabled?: boolean }>(({ theme, disabled = false }) => ({
  display: 'flex',
  ...
  backgroundColor: disabled ? theme.palette.neutral.light : theme.palette.background.default,
  '&:hover': {
...
export const StyledCardTitle = styled('h3')<{ lines?: number }>(({ theme, lines = 2 }) => ({
...
export const StyledParagraphInfo = styled('p')<{ disabled?: boolean }>(({ theme, disabled = false }) => ({
