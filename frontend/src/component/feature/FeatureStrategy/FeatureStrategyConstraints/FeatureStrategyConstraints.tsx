...
import { UPDATE_FEATURE_STRATEGY, CREATE_FEATURE_STRATEGY } from 'component/providers/AccessProvider/permissions';
...
const filterConstraints = (constraint: any) => {
  if (constraint.hasOwnProperty('values') && (!constraint.hasOwnProperty('value') || constraint.value === '')) {
...
export const FeatureStrategyConstraints = ({
  ...
  const setConstraints = (value: React.SetStateAction<IConstraint[]>) => {
    setStrategy((prev) => {
      return {
        ...prev,
        constraints: value instanceof Function ? value(prev.constraints || []) : value,
      };
    });
  };

  const showCreateButton = useHasProjectEnvironmentAccess(CREATE_FEATURE_STRATEGY, projectId, environmentId);

  const allowEditAndDelete = useHasProjectEnvironmentAccess(UPDATE_FEATURE_STRATEGY, projectId, environmentId);
