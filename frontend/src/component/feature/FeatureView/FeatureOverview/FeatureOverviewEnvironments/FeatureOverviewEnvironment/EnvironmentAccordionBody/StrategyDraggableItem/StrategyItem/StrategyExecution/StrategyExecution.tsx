...
const StyledContainer = styled(Box, {
  ...
  '.constraint-icon': {
    fill: disabled ? theme.palette.neutral.light : theme.palette.common.white,
  },
}));
    ...
    return Object.keys(parameters).map((key) => {
      switch (key) {
        ...
        case 'UserIds': {
          ...
          return <ConstraintItem key={key} value={users} text='user' />;
        }
        case 'hostNames':
        case 'HostNames': {
          const hosts = parseParameterStrings(parameters[key]);
          return <ConstraintItem key={key} value={hosts} text={'host'} />;
        }
        case 'IPs': {
  ...
  const customStrategyList = useMemo(() => {
    ...
    return definition?.parameters.map((param) => {
      ...
      switch (param?.type) {
          ...
          return values.length > 0 ? (
            <StyledValueContainer>
              ...
              <StyledValueSeparator>
                has {values.length} {values.length > 1 ? `items` : 'item'}:{' '}
        ...
        case 'boolean':
          return parameters[name] === 'true' || parameters[name] === 'false' ? (
            <StyledValueContainer>
              <StringTruncator maxLength={15} maxWidth='150' text={name} />
              {isSetTo}
