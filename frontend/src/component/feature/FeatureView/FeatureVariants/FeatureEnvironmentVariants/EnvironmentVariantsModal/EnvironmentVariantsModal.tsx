...
const StyledName = styled('span', {
  ...
  color: deprecated ? theme.palette.text.secondary : theme.palette.text.primary,
...
export const EnvironmentVariantsModal = ({
  ...
  const formatApiCode = () =>
    isChangeRequest
      ? `curl --location --request POST '${uiConfig.unleashUrl}/api/admin/projects/${projectId}/environments/${environment?.name}/change-requests' \\
    --header 'Authorization: INSERT_API_KEY' \\
    --header 'Content-Type: application/json' \\
    --data-raw '${JSON.stringify(crPayload, undefined, 2)}'`
      : `curl --location --request PATCH '${uiConfig.unleashUrl}/api/admin/projects/${projectId}/features/${featureId}/environments/${environment?.name}/variants' \\
    --header 'Authorization: INSERT_API_KEY' \\
    --header 'Content-Type: application/json' \\
    ...
    <SidebarModal open={open} onClose={handleClose} label=''>
      ...
      >
        ...
        <StyledForm onSubmit={handleSubmit}>
          ...
          {!hasChangeRequestInReviewForEnvironment && isChangeRequest && (
            <StyledCRAlert severity='info'>
              <strong>Change requests</strong> are enabled
              {environment ? ` for ${environment.name}` : ''}. Your changes need
            ...
            {variantsEdit.map((variant) => (
              <MemoizedVariantForm
                ...
                removeVariant={() =>
                  setVariantsEdit((variantsEdit) =>
                    updateWeightEdit(
                      variantsEdit.filter((v) => v.id !== variant.id),
                      1000,
                    ),
                  )
                }
          ...
          {variantsEdit.length > 0 ? (
            <>
              ...
              <StyledDescription>
                By overriding the stickiness you can control which parameter is
                used to ensure consistent traffic allocation across variants.{' '}
                <Link
              ...
              <div>
                <StyledStickinessSelect
                  ...
                  label={''}
                  editable
                  onChange={(e) => onStickinessChange(e.target.value)}
                />
              </div>
            </>
          ) : (
