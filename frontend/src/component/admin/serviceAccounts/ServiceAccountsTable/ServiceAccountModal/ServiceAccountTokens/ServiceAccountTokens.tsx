...
export const ServiceAccountTokens = ({
  ...
  const onDeleteClick = async () => {
    if (selectedToken) {
      try {
        await deleteServiceAccountToken(serviceAccount.id, selectedToken?.id);
        refetch();
        refetchTokens();
        setDeleteOpen(false);
        setToastData({
    ...
    <>
      ...
      {rows.length === 0 &&
        (searchValue?.length > 0 ? (
          <TablePlaceholder>
            No tokens found matching &ldquo;{searchValue}&rdquo;
          </TablePlaceholder>
        ) : (
          <StyledTablePlaceholder>
            ...
            <StyledPlaceholderSubtitle>
              Create a service account token for access to the Ganpa API.
            </StyledPlaceholderSubtitle>
      ...
      >
        <Typography>
          ...
          to access the Ganpa API. You cannot undo this action.
        </Typography>
      </Dialogue>
    </>
  );
