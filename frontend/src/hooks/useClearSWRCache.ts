...
export const clearCacheEntries = (cache: Cache, currentKey: string, clearPrefix: string, SWR_CACHE_SIZE = 1) => {
  const keys = [...cache.keys()];

  const filteredKeys = keys.filter((key) => key.startsWith(clearPrefix) && key !== currentKey);
...
/**
 ...
 by the `clearPrefix`
 */
export const useClearSWRCache = (currentKey: string, clearPrefix: string, SWR_CACHE_SIZE = 1) => {
