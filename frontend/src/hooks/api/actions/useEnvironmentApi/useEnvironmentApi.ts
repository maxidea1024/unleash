...
const useEnvironmentApi = () => {
  ...
  const validateEnvName = async (envName: string) => {
    ...
    const req = createRequest(path, { method: 'POST', body: JSON.stringify({ name: envName }) }, 'validateEnvName');
  ...
  const createEnvironment = async (payload: IEnvironmentPayload) => {
    ...
    const req = createRequest(path, { method: 'POST', body: JSON.stringify(payload) }, 'createEnvironment');
  ...
  const updateEnvironment = async (name: string, payload: IEnvironmentEditPayload) => {
    const path = `api/admin/environments/update/${name}`;
    const req = createRequest(path, { method: 'PUT', body: JSON.stringify(payload) }, 'updateEnvironment');

    return makeRequest(req.caller, req.id);
  };

  const cloneEnvironment = async (name: string, payload: IEnvironmentClonePayload) => {
    const path = `api/admin/environments/${name}/clone`;
    const req = createRequest(path, { method: 'POST', body: JSON.stringify(payload) }, 'cloneEnvironment');
  ...
  const changeSortOrder = async (payload: ISortOrderPayload) => {
    ...
    const req = createRequest(path, { method: 'PUT', body: JSON.stringify(payload) }, 'changeSortOrder');
...
export const createSortOrderPayload = (environments: Readonly<IEnvironment[]>): ISortOrderPayload => {
