...
describe('Last three unique flags are persisted and duplicates are skipped', () => {
  ...
  it('only persists the last three unique flags across components and skips duplicates', async () => {
    ...
    expect(await screen.findAllByText('Feature2 - Project2')).toHaveLength(4);
    expect(await screen.findAllByText('Feature3 - Project3')).toHaveLength(4);
    expect(await screen.findAllByText('Feature4 - Project4')).toHaveLength(4);
  });
});
