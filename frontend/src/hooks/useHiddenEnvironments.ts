...
export const useHiddenEnvironments = () => {
  ...
  const { value: globalStore, setValue: setGlobalStore } = useGlobalLocalStorage();
  const [hiddenEnvironments, setStoredHiddenEnvironments] = useState<Set<string>>(
    new Set(globalStore.hiddenEnvironments),
  );

  const setHiddenEnvironments = (environment: string) => {
    setGlobalStore((params) => {
      const hiddenEnvironments = new Set(Array.from(params.hiddenEnvironments || []));
