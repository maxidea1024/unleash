...
import { responseTimeMetrics, storeRequestedRoute } from './response-time-metrics';
...
describe('responseTimeMetrics new behavior', () => {
  ...
  test.each([undefined, '/'])('reports (hidden) when route is undefined and path is %s', async (path: string) => {
    let timeInfo: any;
    // register a listener
    eventBus.on(REQUEST_TIME, (data) => {
      timeInfo = data;
