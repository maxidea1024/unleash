import { ADMIN, CREATE_FEATURE, DELETE_FEATURE, UPDATE_FEATURE, UPDATE_PROJECT_SEGMENT } from '../types/permissions';
...
export function findParam(name: string, { params, body }: Request, defaultValue?: string): string | undefined {
...
const rbacMiddleware = (
  ...
  { featureToggleStore, segmentStore }: Pick<IUnleashStores, 'featureToggleStore' | 'segmentStore'>,
  ...
  return (req, res, next) => {
    req.checkRbac = async (permissions: string | string[]) => {
      ...
      const environment = findParam('environment', req) || findParam('environmentId', req);

      // Temporary workaround to figure out projectId for feature flag updates.
      // will be removed in Unleash v5.0
      if (!projectId && permissionsArray.some((permission) => [DELETE_FEATURE, UPDATE_FEATURE].includes(permission))) {
        const { featureName } = params;
        projectId = await featureToggleStore.getProjectId(featureName);
      } else if (
        projectId === undefined &&
        permissionsArray.some((permission) => permission === CREATE_FEATURE || permission.endsWith('FEATURE_STRATEGY'))
      ) {
        projectId = 'default';
      }
      ...
      if (!projectId && permissionsArray.includes(UPDATE_PROJECT_SEGMENT) && params.id) {
        const { id } = params;
