...
export const extractUserIdFromUser = (user: IUser | IApiUser) =>
  (user as IUser)?.id || (user as IApiUser)?.internalAdminTokenUserId || SYSTEM_USER.id;

export const extractUserId = (req: IAuthRequest | IApiRequest) => extractUserIdFromUser(req.user);
...
export const extractAuditInfoFromUser = (user: IUser | IApiUser, ip: string = '127.0.0.1'): IAuditUser => ({
  id: extractUserIdFromUser(user),
  username: extractUsernameFromUser(user),
  ip,
});

export const extractAuditInfo = (req: IAuthRequest | IApiRequest): IAuditUser => ({
  id: extractUserId(req),
  username: extractUsername(req),
  ip: req.ip,
});
