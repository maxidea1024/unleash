...
test('Can send order environments email', async () => {
  ...
  const content = await emailService.sendOrderEnvironmentEmail('user@user.com', customerId, environments);
  expect(content.from).toBe('noreply@getunleash.ai');
  expect(content.subject).toBe('Unleash - ordered environments successfully');
  expect(content.html.includes(`<li>Name: ${environments[0].name}, Type: ${environments[0].type}</li>`)).toBe(true);
  expect(content.html.includes(`<li>Name: ${environments[1].name}, Type: ${environments[1].type}</li>`)).toBe(true);
