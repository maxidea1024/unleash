...
import { type Context, InMemStorageProvider, Unleash, UnleashEvents } from 'unleash-client';
import { ApiTokenType } from '../../types/models/api-token';
import { type FrontendSettings, frontendSettingsKey } from '../../types/settings/frontend-settings';
...
export type Services = Pick<
  IUnleashServices,
  'featureToggleServiceV2' | 'clientMetricsServiceV2' | 'settingService' | 'configurationRevisionService'
>;
...
export class FrontendApiService {
   ...
   */
  private readonly clients: Map<ApiUser['secret'], Promise<Unleash>> = new Map();
  ...
  async getFrontendApiFeatures(token: IApiUser, context: Context): Promise<FrontendApiFeatureSchema[]> {
  ...
  async registerFrontendApiMetrics(token: IApiUser, metrics: ClientMetricsSchema, ip: string): Promise<void> {
    FrontendApiService.assertExpectedTokenType(token);

    const environment = this.services.clientMetricsServiceV2.resolveMetricsEnvironment(token as ApiUser, metrics);
  ...
  private async createClientForFrontendApiToken(token: IApiUser): Promise<Unleash> {
  ...
  async setFrontendSettings(value: FrontendSettings, auditUser: IAuditUser): Promise<void> {
    const error = validateOrigins(value.frontendApiOrigins);
    if (error) {
      throw new BadDataError(error);
    }
    await this.services.settingService.insert(frontendSettingsKey, value, auditUser, false);
  }

  async fetchFrontendSettings(): Promise<FrontendSettings> {
    try {
      this.cachedFrontendSettings = await this.services.settingService.get(frontendSettingsKey, {
