...
export default class FeatureSearchStore implements IFeatureSearchStore {
  ...
  private buildRankingSql(
    ...
    if (sortBy.startsWith('environment:')) {
      ...
      rankingSql += `${this.db.raw(`coalesce(${lastSeenQuery}, features.last_seen_at) ${validatedSortOrder} nulls last`).toString()}, features.created_at asc, features.name asc`;
    } else if (sortByMapping[sortBy]) {
      rankingSql += `${this.db.raw(`?? ${validatedSortOrder}`, [sortByMapping[sortBy]]).toString()}, features.created_at asc, features.name asc`;
    } else {
