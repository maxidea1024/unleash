...
import type { IAccessStore, IAccountStore, IEvent, IEventStore, IOnboardingReadModel, MinimalUser } from '../../types';
...
export class PersonalDashboardService {
  ...
  async getProjectOwners(userId: number): Promise<UserProjectOwner[]> {
    ...
    const filter = accessibleProjects.mode === 'all' ? undefined : new Set(accessibleProjects.projects);
  ...
  async getPersonalProjectDetails(
    ...
    const [latestEvents, owners, roles, healthScores] = await Promise.all([
      ...
      this.accessStore.getAllProjectRolesForUser(userId, projectId).then(filterRoles),
    ...
    if (healthScores.length >= 4) {
      avgHealthCurrentWindow = Math.round(healthScores.slice(0, 4).reduce((acc, score) => acc + score, 0) / 4);
    }

    if (healthScores.length >= 8) {
      avgHealthPastWindow = Math.round(healthScores.slice(4, 8).reduce((acc, score) => acc + score, 0) / 4);
    }
